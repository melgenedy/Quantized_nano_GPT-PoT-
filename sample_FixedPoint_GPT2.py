...
max_new_tokens = 100 #500 # number of tokens generated in each sample
...
def get_GPT_quantization_config(
    ...
    weight_qmin: int = -2**25+1,#-2**31+1,#-127,
    weight_qmax: int = 2**25-1,#2**31-1,#127,
):
    ...
    extra_args: Dict[str, Any] = {"eps": 2**-22}
    if is_qat:
...
loss_iters = 100
out_loss_org_final = torch.zeros(loss_iters)
out_loss_pot_final = torch.zeros(loss_iters)
# run generation
with torch.no_grad():
    with ctx:
        for k in range(loss_iters):
            
            out_loss = estimate_loss()
            print('loss = ', out_loss[0], 'loss_quantized = ', out_loss[1])
            #X, Y = get_batch('test')
            out_loss_org_final[k] = out_loss[0]
            out_loss_pot_final[k] = out_loss[1]

print('loss avg = ', torch.mean(out_loss_org_final), 'loss_quantized avg = ', torch.mean(out_loss_pot_final))
