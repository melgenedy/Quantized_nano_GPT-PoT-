...
def get_nanoGPT_quantization_config(
    ...
    weight_qmin: int = -15,#-2**31+1,#-127,
    weight_qmax: int = 15,#2**31-1,#127,
):
...
quantizer = XNNPACKQuantizer().set_global(get_nanoGPT_quantization_config())
...
def round_to_power_of_2(x):
    ...
    if 2**power < abs_x:
        power +=1
    return 2**power * (1 if x > 0 else -1)

quantized_weights = m_quant.state_dict()
for name, param in quantized_weights.items():
    if isinstance(param, torch.Tensor) and param.dtype == (torch.int8):
